...
|---|---|---|---|
| **Заменить Ribbon на classic-style menu bar** (File/Edit/View/Insert/Format/Slide/Arrange/Tools) с пунктами как в GSlides | ⛔ нет | **P0** | 3-5 дней |
...
| Кнопка "Slideshow" / "Present" справа сверху | ✅ есть (в Ribbon → Показ) | переехать в menu bar | 0.5 дня |
...
| Find & replace (всё за слайдами) | ⛔ нет | P0 | 2 дня |
...
| Что | Статус | Приоритет | Усилия |
|---|---|---|---|
| **Align**: 6 кнопок (left/center/right/top/middle/bottom) на selection или на slide | ⛔ нет | **P0** | 1 день |
| **Distribute**: horizontal / vertical | ⛔ нет | **P0** | 0.5 дня |
| **Group / Ungroup** (Cmd+G / Cmd+Shift+G) | ⛔ нет | **P0** | 2 дня (модель group + render) |
| Rotate 90° CW/CCW, Flip H/V | ⛔ нет | P0 | 0.5 дня |
